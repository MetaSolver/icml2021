...
if __name__=="__main__":
    ...
    for itr in range(args.nepochs_nn * batches_per_epoch):
        ...
        ##### Forward pass
        if is_odenet:
            logits = model(x, train_solvers, train_solver_options)
        else:
            logits = model(x)
...
# CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=4 python3 runner.py --data_root /workspace/raid/data/datasets --save ./experiment2_new --network 'odenet' --downsampling-method 'conv' --solvers "rk2,u,16,-1,0.5,-1;rk2,u,16,-1,1.0,-1" --solver_mode "switch" --activation "relu" --seed 702  --nepochs_nn 160 --nepochs_solver 0 --nstages 1 --lr 0.1 
