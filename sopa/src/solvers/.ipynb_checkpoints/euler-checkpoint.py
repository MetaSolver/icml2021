...
class RKOrder2Stage2(RKParametricSolver):
    ...
    def sample_params(self, l = 0., r = 1.):
        self.u = (r - l) * torch.rand(1) + l
        
        if self.v is not None:
            self.v = (r - l) * torch.rand(1) + l
            
        self.build_ButcherTableau()
        
        
    def noise_params(self, mean_u, mean_v = None, std = 0., p = 0.):
        ''' Noise solver paramers with Normal noise with probability p
        '''
        d = torch.distributions.Bernoulli(torch.tensor([p]))
        
        if d.sample():
            std = torch.tensor((std,))

            self.u = torch.normal(mean = torch.tensor((mean_u,)), std = std)
            if self.v is not None:
                self.v = torch.normal(mean = torch.tensor((mean_v,)), std = std)

        else:
            self.u = torch.tensor((mean_u,))
            if self.v is not None:
                self.v = torch.tensor((mean_v,))
